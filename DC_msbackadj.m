function Y = DC_msbackadj(X,Y,varargin)
...
%   $Revision: 1.1.12.8 $  $Date: 2010/09/20 14:33:10 $
...
% check inputs
DC_bioinfochecknargin(nargin,2,mfilename);
% set defaults
stepSize = 200;
windowSize = 200;
                    ...
                    if bioinfoprivate.opttf(pval) 
                        if isnumeric(pval)
                            if isscalar(pval)
                                ...
                                'SHOWPLOT must be a single index to one of the signals in Y or a logical. Plotting the first column in Y only.')
                            end 
                        else
                            plotId = 1;
                        end
...
if ~isnumeric(Y) || ~isreal(Y)
   error('Bioinfo:msbackadj:IntensityNotNumericAndReal',...
       'Intensities must be numeric and real.') 
end

if ~isnumeric(X) || ~isreal(X)
   error('Bioinfo:msbackadj:XNotNumericAndReal',...
       'X scale must be numeric and real.') 
end

if size(X,1) ~= size(Y,1)
   error('Bioinfo:msbackadj:NotEqualNumberOfSamples',...
       'The intensity vector(s) must have the same number of samples as the X scale.')
end
...
if (plotId~=0) && ~any((1:numSignals)==plotId)
    ...
    'SHOWPLOT value is not a valid signal index, no plot was generated.')
end
...
if size(X,2)>1
    ...
    if size(X,2) ~= numSignals
        error('Bioinfo:msbackadj:NotEqualNumberOfXScales',...
        'A X scale must be supplied for every intensity vector when size(X,2)>1.')
    end
end
...
if ~multiple_X
    ...
    while Xpid <= Xend
        ...
        if id > maxNumWindows
            ...
            'Maximum number of windows exceeded, verify the STEP value or function handle.')
        end
    end
    numWindows = id-1;
end
...
if nargout>0 || (ns == plotId)
    ...
    if ~isequal('none',smoothMethod)
        WE(1:numWindows) = ...
            masmooth(Xpt+Xw/2,WE(1:numWindows),10,smoothMethod,2);
    end
